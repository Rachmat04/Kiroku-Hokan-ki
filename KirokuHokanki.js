...
(function () {
  ...
  class ArchiveUIManager {
    ...
    /**
     * Loads the gadget's stylesheet from its dedicated page on id.wikipedia.org.
     *
     * The stylesheet was moved out of this file in v2.13.0 to keep styling
     * separate from application logic. The URL is fixed (not built from the
     * local wiki) because the CSS page lives on id.wikipedia.org regardless
     * of which wiki this gadget is running on. Loaded once via mw.loader.load();
     * the browser cache handles repeat loads across pages.
     */
    injectUtilityStyles() {
      const cssUrl =
        "https://id.wikipedia.org/w/index.php?title=Pengguna:Rachmat04/KirokuHokanki.css&action=raw&ctype=text/css";
